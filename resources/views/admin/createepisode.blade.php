    ...
    <div class="container my-4">
        ...
        <form method="post" enctype="multipart/form-data" action="{{ route('createepisode') }}">
            ...
            <div class="input-group my-3 flex-nowrap">
                ...
                <input value="{{ $episode ? $episode->episode : old('episode') }}" name='episode' type="text" class="form-control"
                    placeholder="Episode" aria-label="episode" aria-describedby="addon-wrapping">
            </div>
            @if ($episode && $episode->video)
                <input type="hidden" value="{{ $episode->video }}" name="existedvideo">
                <img src="{{ $episode->video }}" class="uploaded-img mb-4" alt="{{ $episode->id }}">
            @endif
            @error('video')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
